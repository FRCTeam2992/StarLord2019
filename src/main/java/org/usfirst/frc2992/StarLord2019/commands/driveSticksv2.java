 ...
 *
 */
public class driveSticksv2 extends Command {
    ...
    protected void execute() {
        ...
        if (right > straightThreshold && left > straightThreshold) {
            ...
            if (!straightDrive) {
                ...
                straightHead = scaleAngle(Robot.driveTrain.navx.getYaw());
                straightDrive = true;
            } 
            else {
                // Not first cycle so we will adjust drive powers
                right += gkp * calcGyroError(straightHead);
                left -= gkp * calcGyroError(straightHead);
            }
        }
    ...
    private double calcGyroError(double heading){ //For PID- finding error
        double error = 0;
        error = scaleAngle(Robot.driveTrain.navx.getYaw()) - heading;
        if(Math.abs(error) >= 1){
            return error;
        } else {
            return 0;
